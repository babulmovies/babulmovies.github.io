...
angular.module("BabulmoviesApp", ['MyUtils', 'ngRoute', 'ngResource', 'ngAnimate', 'ngSanitize'], function ($locationProvider) {
    ...
    .controller("MovieController", ['$scope', '$http', '$location',
        function ($scope, $http, $location) {
            $scope.movie = {};
            $scope.hasTrailer = hasTrailer;
            $scope.hasWatch = hasWatch;
            $scope.hasDownload = hasDownload;
            ...
            function hasTrailer() {
                if ($scope.movie) {
                    if ($scope.movie.trailer && $scope.movie.trailer.length > 0) {
                        return true;
                    }
                }
                return false;
            ...
            function hasWatch() {
                if ($scope.movie) {
                    if ($scope.movie.watch && $scope.movie.watch.length > 0) {
                        return true;
                    }
                }
                return false;
            ...
            function hasDownload() {
                if ($scope.movie) {
                    if ($scope.movie.download && $scope.movie.download.length > 0) {
                        return true;
                    }
                }
                return false;
